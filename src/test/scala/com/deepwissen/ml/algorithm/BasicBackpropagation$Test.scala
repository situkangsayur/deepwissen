...
class BasicBackpropagation$Test extends FunSuite {
  ...
  val parameterBreastCancer  = BackpropragationParameter(
    hiddenLayerSize = 1,
...
//
  ...
  test("traininig and classification and save model for breast cancer") {
    // training
    try {

//      logger.info(finalDataSetBreastCancer.toString())

      val network = BasicBackpropagation.train(finalDataSetBreastCancer, parameterBreastCancer)

      val validator = BackProValidation()

      val result = validator.classification(network, BasicClassification, finalDataSetBreastCancer, SigmoidFunction)
      logger.info("result finding : "+ result.toString())
      ...
      finalDataSetBreastCancer.foreach { data =>
        ...
        realScore.asInstanceOf[BinaryValue].get.zipWithIndex.foreach(p => {
          val percent = Math.round(p._1 * 100.0)
          val score = if (p._1 > 0.7) 1.0 else 0.0
          val originalClass = data(targetClassBreastCancer).asInstanceOf[BinaryValue].get(p._2)
          println(s"real $p== percent $percent% == score $score == targetClass ${originalClass}")
          assert(score == originalClass)
        })
      }
